...
public class SendMail extends AppCompatActivity {
    ...
    private void sendMail() {
        ...
        if (recipients.length == 0 || subject.isEmpty() || message.isEmpty()) {
            Toast.makeText(SendMail.this, "All fields are required", Toast.LENGTH_SHORT).show();
        } else {
            for (String recipient : recipients) {
                if (!isValidEmail(recipient.trim())) {
                    Toast.makeText(SendMail.this, "Invalid email format for recipient: " + recipient, Toast.LENGTH_SHORT).show();
                    return;
                }
            }
